...
# }
# Calculate net parameters for current and future use
calculate_net_parameters <- function(bednet_params, target_dn0) {
  differences <- abs(bednet_params$dn0 - target_dn0)
  closest_index <- which.min(differences)
  bednet_params[closest_index, ]
}

# Determine target net distribution based on current and future usage
targetted_distribution <- function(current_use, target_use) {
  current_use <- max(0, min(1, current_use))
  target_use <- max(current_use, min(1, target_use))
  
  prop_uncovered <- 1 - current_use
  target_difference <- target_use - current_use
  target_difference / prop_uncovered
}

# Generate ITN distribution timings
fetch_net_distribution_times <- function(DOY_mass_campaigns, routine_interval, routine_topups_per_year, sim_length, year) {
  routine_topups_DOY <- round(seq(DOY_mass_campaigns, by = routine_interval, length.out = routine_topups_per_year))
  routine_topups_DOY <- sort(routine_topups_DOY %% year)
  rep_years <- rep(seq(0, ceiling(sim_length/year)), each = routine_topups_per_year)
  net_times <- routine_topups_DOY + rep_years * year
  N_dist <- sum(net_times < sim_length)
  net_times[1:N_dist]
}
...
# Identify ITN distributions for mass campaigns
fetch_mass_campaign_times <- function(DOY_mass_campaigns, first_year, final_year, interval, year) {
  DOY_mass_campaigns + year * seq(first_year, final_year, interval)
}

# Generate number of nets to be distributed at each distribution event
fetch_nets_distributed <- function(N_dist, net_times, historic_mass_campaigns, future_mass_campaigns, lambda, routine_interval,
                                   prop_historic_routine, prop_future_routine, historic_max_usage, future_max_usage,
                                   lhs_sample, routine_topups_per_year, year, historic_routine, future_routine) {
  
  calculate_routine_top_up <- function(routine_usage) {
    if (routine_usage > 0) {
      monthly_routine_loss <- routine_usage * (1 - exp(-(year/routine_topups_per_year) * lambda))
      monthly_routine_loss / (1 - routine_usage - monthly_routine_loss)
    } else {
      0
    }
  }
  ...
  # Calculate routine top-ups
  historic_routine_top_up <- if (historic_routine) calculate_routine_top_up(prop_historic_routine(lhs_sample$itn_use)) else 0
  future_routine_top_up <- if (future_routine) calculate_routine_top_up(prop_future_routine(lhs_sample$itn_future)) else 0
  
  E_nets <- numeric(N_dist)
  nets_distributed <- numeric(N_dist)
  
  for (i in seq_len(N_dist)) {
    pre_distribution <- if (i == 1) 0 else E_nets[i - 1] * exp(-routine_interval * lambda)
    
    if (net_times[i] %in% historic_mass_campaigns) {
      nets_distributed[i] <- targetted_distribution(pre_distribution, historic_max_usage)
      E_nets[i] <- historic_max_usage
    } else if (net_times[i] %in% future_mass_campaigns) {
      nets_distributed[i] <- targetted_distribution(pre_distribution, future_max_usage)
      E_nets[i] <- future_max_usage
    } else if (net_times[i] < future_mass_campaigns[1]) {
      nets_distributed[i] <- historic_routine_top_up
      E_nets[i] <- pre_distribution + (1 - pre_distribution) * historic_routine_top_up
...
# Main function to set bednet parameters
set_bednet_parameters <- function(simparams, lhs_sample, bednet_params) {
  # Initialize constants and parameters
  year <- 365
  routine_topups_per_year <- 26 # Change as needed
  historic_routine <- TRUE
  DOY_mass_campaigns <- 1
  sim_length <- 12 * year
  N_species <- 1
  invlambda <- 2.1 * year
  lambda <- 1 / invlambda
  ...
  # Define functions for historic and future routines
  prop_historic_routine <- function(prop_any_itn) {0.1562572 * prop_any_itn}
  prop_future_routine <- function(prop_any_itn) {0.1562572 * prop_any_itn}

  # Calculate initial parameters based on lhs_sample and bednet_params
  target_dn0_use <- round(lhs_sample$dn0_use, digits = 3)
  target_dn0_future <- round(lhs_sample$dn0_future, digits = 3)

  selected_net_params_use <- calculate_net_parameters(bednet_params, target_dn0_use)
  selected_net_params_future <- calculate_net_parameters(bednet_params, target_dn0_future)

  # Historic and future mass campaign timings
  historic_mass_campaigns <- fetch_mass_campaign_times(DOY_mass_campaigns, first_year = 0, final_year = 9, interval = 3, year = year)
  future_mass_campaigns <- fetch_mass_campaign_times(DOY_mass_campaigns, first_year = 9, final_year = 9, interval = 3, year = year)

  # Calculate maximum usages
  historic_max_usage <- lhs_sample$itn_use
  future_max_usage <- lhs_sample$itn_future

  # Generate number of nets distributed and timings
  net_times <- fetch_net_distribution_times(DOY_mass_campaigns, routine_interval, routine_topups_per_year, sim_length, year)
  N_dist <- length(net_times)
  nets_distributed <- fetch_nets_distributed(N_dist, net_times, historic_mass_campaigns, future_mass_campaigns, lambda, routine_interval,
                                             prop_historic_routine, prop_future_routine, historic_max_usage, future_max_usage,
                                             lhs_sample, routine_topups_per_year, year, historic_routine, lhs_sample$routine != 0)
  
  N_historic_dist <- which(net_times == future_mass_campaigns[1]) - 1
  N_future_dist <- N_dist - N_historic_dist

  # Prepare matrices of parameters for historic and future bednets
  dn0_mat <- matrix(rep(c(rep(selected_net_params_use$dn0, N_historic_dist),
                          rep(selected_net_params_future$dn0, N_future_dist)), N_species),
                    nrow = N_dist, ncol = N_species)
  rn0_mat <- matrix(rep(c(rep(selected_net_params_use$rn0, N_historic_dist),
                          rep(selected_net_params_future$rn0, N_future_dist)), N_species),
                    nrow = N_dist, ncol = N_species)
  rnm_mat <- matrix(rep(0.24, N_dist * N_species), nrow = N_dist, ncol = N_species)
  gamman_vec <- c(rep(selected_net_params_use$gamman, N_historic_dist),
                  rep(selected_net_params_future$gamman, N_future_dist))
